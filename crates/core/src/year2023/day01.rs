...
fn calibration_value(line: &str, part2: bool) -> u32 {
    ...
    for byte in line.bytes() {
        ...
        let digit = match (bits & 0xff, bits & m1, bits & m2, bits & m3) {
            ...
            (0b111000, _, _, _) => 8,
            (0b111001, _, _, _) => 9,
            (_, 0b11011110110111001100101, _, _) => 1,
            (_, 0b11101000111011101101111, _, _) => 2,
            (_, _, _, 0b111010001101000011100100110010101100101) => 3,
            (_, _, 0b1100110011011110111010101110010, _) => 4,
            (_, _, 0b1100110011010010111011001100101, _) => 5,
            (_, 0b11100110110100101111000, _, _) => 6,
            (_, _, _, 0b111001101100101011101100110010101101110) => 7,
            (_, _, _, 0b110010101101001011001110110100001110100) => 8,
            (_, _, 0b1101110011010010110111001100101, _) => 9,
            _ => {
                continue;
            }
        };
