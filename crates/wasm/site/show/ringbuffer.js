...
export function ReaderWithBuffer(sharedArrayBuffer, offset, length) {
  ...
  this.wantMore = () => {
      ...
      const available = readerOffset > writerOffset ? (writerOffset - readerOffset + dataBuffer.length) : (writerOffset - readerOffset);

      if (available < dataBuffer.length/3) {
        Atomics.store(headerBuffer, HEADER_READER_WANT_MORE_OFFSET, 1);
        Atomics.notify(headerBuffer, HEADER_READER_WANT_MORE_OFFSET);
      } else {
        console.log('not requesting more as utilisation=' + (available*1.0/dataBuffer.length));
      }
  };
}
